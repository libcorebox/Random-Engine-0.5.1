...
class CharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		{
			if ((touch.overlaps(imageInputText)
				|| touch.overlaps(healthIconInputText)
				|| touch.overlaps(animationInputText)
				|| touch.overlaps(animationNameInputText)
				|| touch.overlaps(animationIndicesInputText))
				&& touch.justPressed)
			{ // overlaps check shit for keyboard appearing on android
		...
		if (!charDropDown.dropPanel.visible)
		{
			...
			if (FlxG.keys.pressed.I || FlxG.keys.pressed.J || FlxG.keys.pressed.K || FlxG.keys.pressed.L #if android || _virtualpad.buttonUp.justPressed
				|| _virtualpad.buttonDown.justPressed || _virtualpad.buttonLeft.justPressed || _virtualpad.buttonRight.justPressed
				&& _virtualpad.buttonC.justPressed #end)
			{
