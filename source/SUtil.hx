...
class SUtil
{
	...
	static public function doTheCheck()
	{
			...
			|| !grantedPermsList.contains(Permissions.WRITE_EXTERNAL_STORAGE))
		{
			...
			{
				SUtil.applicationAlert("Permissions",
					"The Game can't run without storage permissions please grant them in app settings" + "\n" + "Press Ok To Close The App");
			}
		}
		...
		{
			SUtil.applicationAlert("Instructions:",
				"You have to copy assets/assets from apk to your internal storage app directory "
				+ "( here "
				+ SUtil.getPath()
				+ " )"
		...
		{
			SUtil.applicationAlert("Instructions:",
				"You have to copy assets/mods from apk to your internal storage app directory "
				+ "( here "
				+ SUtil.getPath()
				+ " )"
				+ "if you hadn't have Zarhiver Downloaded, download it and enable the show hidden files option to have the folder visible"
